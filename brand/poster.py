...
HERO (default):  python brand/poster.py brand/out/final/house-hero.png
...
FORMS:  python brand/poster.py brand/out/final forms
  For every <form>-hero.png in the folder (couch, fridge, map, message, truck) writes img/house-<form>.webp,
  900x675, alpha crop centred in a 4:3 box. These are the stills the inner pages use: the formation that
  matches the page, no WebGL.
"""
import sys, os, glob
from PIL import Image, ImageDraw, ImageFilter

SRC = sys.argv[1]
MODE = sys.argv[2] if len(sys.argv) > 2 else "hero"
...
def formation_posters(folder):
    for src in sorted(glob.glob(os.path.join(folder, "*-hero.png"))):
        form = os.path.basename(src).replace("-hero.png", "")
        if form == "house":
            continue
        im = Image.open(src).convert("RGBA")
        bb = im.getchannel("A").point(lambda v: 255 if v > 8 else 0).getbbox()
        im = im.crop((max(0, bb[0] - 30), max(0, bb[1] - 30), bb[2] + 30, bb[3] + 30))
        w, h = im.size
        nw, nh = (w, round(w * 0.75)) if w / h >= 4 / 3 else (round(h * 4 / 3), h)
        canvas = Image.new("RGBA", (nw, nh), (0, 0, 0, 0))
        canvas.paste(im, ((nw - w) // 2, (nh - h) // 2))
        canvas = canvas.resize((900, 675), Image.LANCZOS)
        out = os.path.join(IMG, f"house-{form}.webp")
        canvas.save(out, quality=84, method=6)
        print(form, canvas.size, os.path.getsize(out))
